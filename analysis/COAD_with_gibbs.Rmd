...
```{r construct-priors}
sorted_unique <- sort(unique(sc_lipid$stage))
priors <- as.numeric(plyr::mapvalues(sc_lipid$stage, from = sorted_unique, to = seq_along(sorted_unique)))
priors[is.na(priors)] <- mean(priors, na.rm = TRUE)
```

```{r gibbs-sample-lipid}
gibbs <- phenot(y, x, q = priors, iter = 2000)
```
